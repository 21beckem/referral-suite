(async () => {
	...
	const person = await idToReferral(getCookieJSON('linkPages')) || null;
	...
	for (let i = 0; i < ThingsToComplete.length; i++) {
		...
		inputsOutput += '<label for="' + elShort + '">' + elShort.toTitleCase() + ' </label><br><input id="completerInputFor_'+el+'" class="completerInputCls" autocomplete="nope" name="' + elShort + '" type="text"><br>';
	}
	newTmpMssg = newTmpMssg.replace(/(?:\r\n|\r|\n)/g, '<br>');
	_('MessageOutput').innerHTML = newTmpMssg;
	_('completerItemsParent').innerHTML = inputsOutput;

	document.getElementsByClassName('completerInputCls').forEach(el => {
		el.oninput = function() {
			document.getElementsByClassName('completerViewThing_{'+this.name.replace(' ', '_')+'}').forEach((x) => {
				x.innerHTML = (this.value == '') ? '{'+this.name+'}' : this.value;
			});
		}
		el.onfocus = function() {
	...
	_('sendTheMessageBtn').onclick = function() {
