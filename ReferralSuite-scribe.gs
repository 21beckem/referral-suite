function doGet(e) {
  ...
  if (readOnlyId != null) {
    if ("fox" in data) {
      if ("new_streak" in data.fox) {
        saveFoxData(area, data.fox.new_streak, readOnlyId, 'streak');
      }
      if ("new_points" in data.fox) {
        saveFoxData(area, data.fox.new_points, readOnlyId, 'points');
      }
    }
...
function saveFoxData(area, data, tabId, key) {
  ...
  let oldFox = decodeFox([sheet.getRange(sMap[area] + 3, 1).getValue(), area]);
  if (key=='streak') {
    if (data != oldFox.streak[0]) {
      oldFox.streak.unshift(data);
    }
  } else {
    oldFox.points += data;
  }
  delete oldFox.new_points;
  delete oldFox.new_streak;
  sheet.getRange(sMap[area] + 3, 1).setValue(CryptoJS.AES.encrypt(JSON.stringify(oldFox), area) + '');
}
...
*/
function decodeFox(t){var r={points:0,streak:[]};let e=!1;if(null==t)return r;if((0==t.length||""==t[0])&&(e=!0),e)return r;try{return JSON.parse(CryptoJS.AES.decrypt(t[0],t[1]).toString(CryptoJS.enc.Utf8))}catch(t){return r}}
